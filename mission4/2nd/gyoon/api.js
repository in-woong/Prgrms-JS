const username = 'gyoon'
const APIurlEndpoint = 'http://todo-api.roto.codes/'
let loading = false

const toggleLoading = (loadingDescription) => {
  loading = !loading
  let loadingGifHTML = ''
  loading ? loadingGifHTML = loadingGifCat() : loadingGifHTML = ''
  document.querySelector('#loading-gif').innerHTML = loadingGifHTML
  loadingDescription ? 
    document.querySelector('#loading-gif-description').innerHTML = loadingDescription : 
    document.querySelector('#loading-gif-description').innerHTML = ''
}

const fetchDataAPI = async function() {
  toggleLoading('fetching and syncing w/<br />most up-to-date todos...')
  const res = await fetch(`${APIurlEndpoint}${username}`)
  toggleLoading()
  return await res.json()
}

const addTodoAPI = async function(todoToAdd) {
  toggleLoading('adding todo...')
  const res = await fetch(`${APIurlEndpoint}${username}`, {
    method: 'POST',
    headers: {
...
const deleteTodoAPI = async function(id) {
  toggleLoading('deleting todo...')
  const res = await fetch(`${APIurlEndpoint}${username}/${id}`, {
    method: 'DELETE',
  })
  toggleLoading()
  return res
...
const toggleTodoIdAPI = async function(id) {
  toggleLoading('toggling todo...')
  const res = await fetch(`${APIurlEndpoint}${username}/${id}/toggle`, {
    method: 'PUT',
  })
  toggleLoading()
  return res
...
// gif loading cat from (probably should pay for this...)
// https://loading.io/spinner/bluecat/-loading-gif-icon-animal
const loadingGifCat = () => {
  return (`
    <svg
      class="lds-bluecat"
    ...
    >
      <g transform="rotate(9.7905 50 50)">
        <animateTransform
      ...
      <g transform="rotate(32.7629 50 50)">
        <animateTransform
          attributeName="transform"
          type="rotate"
          values="360 50 50;0 50 50"
