    ...
    if (!new.target){
      return new TodoList(data)
        ...
        this.render = function () {
            ...
            if (Array.isArray(data)) {
                ...
                if(data.length) {
                    ...
                    if(Object.keys(el).includes("text") && Object.keys(el).includes("isCompleted")){
                        ...
                        throw new Error("data1 요소의 key값으로 text가 존재하지 않습니다")
                    } else if(!Object.keys(el).includes("isCompleted")) {
                        // 배열의 요소가 key값으로 isCompleted를 갖지 않는다면,
                        throw new Error("data1 요소의 key값으로 isCompleted가 존재하지 않습니다")
                    } 
                })
            } else {
                // 주어진 인자가 배열이 아니라면
            ...
            if (!data) {
                throw new Error(`data1는 falsy한 값입니다: ${data}`)
            } else {
