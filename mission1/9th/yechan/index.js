...
const error = {
    ...
    NOTARRAY_DATA: "데이터 타입이 ARRAY가 아닙니다.",
...
const dataValidCheck = (data) => {
    //배열여부 검사
    if (Array.isArray(data) === false) throw new Error(error.NOTARRAY_DATA)
    //text 데이터를 가지고 있는지 여부 검사
    data.forEach(data => {
        // console.log(data)
        if (!data.hasOwnProperty('text')) throw new Error(error.INVALID_DATA)
        if (data.text === (null || undefined)) throw new Error(error.INVALID_DATA)
    })
}
