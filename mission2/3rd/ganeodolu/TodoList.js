function TodoList(listname, data) {
  ...
  this.render = function() {
    ...
    let dataText = Object.values(data)
      .map((val, idx) => {
        ...
        // data.text 내용이 JS를 포함하지 않으며, 문자열이 아닌 경우를 데이터가 이상하다고 가정
          ...
          throw new Error(
            'JS에 관련된 내용이 아니거나, data 타입이 문자열이 아닙니다.'
          )
        }
        // isCompleted 가 참이면 <strike>태그를 넣어서 저장(완료되었다는 의미), false면 <div>만 넣어서 저장
        return val.isCompleted
          ? `<li><span id="line${idx}">완료 <strike>${val.text}</strike></span> ${delButton}</li>`
          : `<li><span id="line${idx}">${val.text}</span> ${delButton}</li>`
      })
      .join('')
